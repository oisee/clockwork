/*
  ayumi AudioWorklet — AY-3-8910 emulation
  Based on: DrSnuggles/AYSir (AudioWorklet adaptation)
  Original: Peter Sovietov (ayumi), Alexander Kovalenko (JS port)
  License: MIT
*/
...
class AYUMI extends AudioWorkletProcessor {
  ...
  constructor() {
    ...
    this.envelope = 0
    this.dacTable = this.AY_DAC_TABLE
    this.step = 0.0
    this.x = 0.0
    this.left = 0.0
    this.right = 0.0
    this.interpolatorLeft = { c: new Float64Array(4), y: new Float64Array(4) }
    this.interpolatorRight = { c: new Float64Array(4), y: new Float64Array(4) }
    this.dcFilterLeft = { sum: 0.0, delay: new Float64Array(this.DC_FILTER_SIZE) }
    this.dcFilterRight = { sum: 0.0, delay: new Float64Array(this.DC_FILTER_SIZE) }
    this.dcIndex = 0
    this.firLeft = new Float64Array(this.FIR_SIZE * 2)
    this.firRight = new Float64Array(this.FIR_SIZE * 2)
    this.firIndex = 0
    this.muted = false
  }
  ...
  Channel() {
    return {
      toneCounter: 0, tonePeriod: 1, tone: 0,
      tOff: 0, nOff: 0, eOn: 0,
      volume: 0, panLeft: 1.0, panRight: 1.0
    }
  }

  getChannels() {
    return [this.Channel(), this.Channel(), this.Channel()]
  }
  ...
  updateTone(index) {
    ...
    if (++ch.toneCounter >= ch.tonePeriod) { ch.toneCounter = 0; ch.tone ^= 1 }
    return ch.tone
  }
  ...
  slideUp(e) { if (++e.envelope > 31) { e.envelopeSegment ^= 1; e.resetSegment() } }
  slideDown(e) { if (--e.envelope < 0) { e.envelopeSegment ^= 1; e.resetSegment() } }
  holdTop(e) {}
  holdBottom(e) {}
  ...
  updateMixer() {
    let noise = this.updateNoise()
    let envelope = this.updateEnvelope()
    this.left = 0
    this.right = 0
    for (let i = 0; i < 3; i++) {
      let out = (this.updateTone(i) | this.channels[i].tOff) & (noise | this.channels[i].nOff)
  ...
  setTone(index, period) { period &= 0xfff; this.channels[index].tonePeriod = (period == 0) | period }
  setNoise(period) { this.noisePeriod = period & 0x1f }
  setMixer(index, tOff, nOff, eOn) {
    this.channels[index].tOff = tOff & 1
    this.channels[index].nOff = nOff & 1
    this.channels[index].eOn = eOn & 1
  }
  setVolume(index, volume) { this.channels[index].volume = volume & 0x0f }
  setEnvelope(period) { period &= 0xffff; this.envelopePeriod = (period == 0) | period }
  ...
  decimate(x) {
    const y = -0.0000046183113992051936 * (x[1] + x[191]) +
      ...
      0.125 * x[96];
    for (let i = 0; i < this.DECIMATE_FACTOR; i++) {
      x[this.FIR_SIZE - this.DECIMATE_FACTOR + i] = x[i]
    }
    return y
  }

  AYUMIprocess() {
    const cLeft = this.interpolatorLeft.c,
      yLeft = this.interpolatorLeft.y,
      cRight = this.interpolatorRight.c,
      yRight = this.interpolatorRight.y,
      firOffset = this.FIR_SIZE - this.firIndex * this.DECIMATE_FACTOR,
      firLeft = this.firLeft.subarray(firOffset),
      firRight = this.firRight.subarray(firOffset)

    this.firIndex = (this.firIndex + 1) % (this.FIR_SIZE / this.DECIMATE_FACTOR - 1)

    for (let i = this.DECIMATE_FACTOR - 1; i >= 0; i--) {
      this.x += this.step
      if (this.x >= 1) {
        this.x--
  ...
  dcFilter(dc, index, x) {
    ...
    dc.delay[index] = x
    return x - dc.sum / this.DC_FILTER_SIZE
  }
  ...
  removeDC() {
    ...
    this.dcIndex = (this.dcIndex + 1) & (this.DC_FILTER_SIZE - 1)
  }
  ...
  handleMessage_(event) {
    ...
    switch (event.data.msg) {
      ...
      case 'regs':
        ...
        if (r[13] != 0xff) this.setEnvelopeShape(r[13])
        break
    }
  }
}

registerProcessor('ayumi-audio-processor', AYUMI)
